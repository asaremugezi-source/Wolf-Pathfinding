...
class animal(entity):
    ...
    def evaluate_moves(self, board, destination):
        ...
        return moves
    def update_eval(self, distance, second_best):
        try:
    ...
    def choose_direction(self, board, moves):
        ...
        if in_local_max:
            ...
            self.update_eval(distance, second_best)
        return best_move
...
class rabbit(animal):
    global wolf_freeze_count
    def flee(self, board, predator):
        if distance_squared(predator.position, self.position) < 64 and wolf_freeze_count > 0:
            self.tile_eval.forget()
            self.predator_position = predator.position
        self.pathfind(board, self.predator_position)

    def evaluate_moves(self, board, predator_position):

        moves = find_available_moves(self.position, board)
        for move in moves + [self.position]:
            value = 1 - 1/(distance_squared(move, predator_position)+1)
            self.tile_eval.initialise(move, value)
        return moves

class wolf(animal):
    
    def hunt(self, board, prey):
        self.pathfind(board, self.current_prey_position)
        if self.position == self.current_prey_position or self.tile_eval.size > 1000:
            self.current_prey_position = prey.position
            self.tile_eval.forget()
...
rabbit1 = rabbit(destination, game_board)
rabbit1.predator_position = wolf1.position
wolf1.current_prey_position = destination
while running:
    destination = rabbit1.position
    keys = pygame.key.get_pressed()
    if keys[pygame.K_SPACE]:
        start = True
    next_mode = display_game(next_animal ,next_mode)
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False
    if start:
        if wolf_freeze_count > 0:
            wolf_freeze_count -= 1
        else:   
            wolf1.hunt(game_board, rabbit1)
        if count % 5 < 4:
            rabbit1.flee(game_board, wolf1)
    
    if wolf1.position == destination and wolf_freeze_count == 0:
        wolf_freeze_count = 1000
        dest_count += 1
    count += 1
    if dest_count > 5:
        break
