...
class animal(entity):
    ...
    def choose_direction(self, board, moves):
        ...
        for move in moves:
            ...
            elif move_value >= max_value  *(1-noise_parameter) + random.uniform(0,noise_parameter) or max_value == 0:
    ...
    for x in range(-8,9):
        for y in range(-8,9):
            ...
            if keys[pygame.K_TAB]:
                dest_count = 0
                index = wolf1.tile_eval.get_tile_index(tile)
                if index != None:
                    normed_list = normalise_list(wolf1.tile_eval.values)
                    
                    if wolf1.tile_eval.distances[index] > 15:
                        next_mode = "rainbow"
...
while running:
    ...
    if wolf1.position == destination:
        print(dest_index,count,destination)
        wolf1.tile_eval.forget()
        if dest_index > 2:
            print(noise_parameter)
            noise_parameter += 1/1024
            dest_index = 0
            if initial:
            ...
            destinations.append(destination)
            dest_index += 1
            destination = (random.randint(-300,300),random.randint(-300,300))
            while not game_board.fetch_tile(destination):
                destination = (random.randint(-1,1) + destination[0], random.randint(-1,1)+destination[1])
