...
class animal(entity):
    ...
    def evaluate_move(self, board, previous_position, destination):
        value = self.tile_eval.value(self.position)
        if (pygame.Vector2(self.position) - pygame.Vector2(previous_position)).magnitude_squared() == 0:
            ...
            self.tile_eval.change(previous_position, .5*value)
        self.tile_eval.decay()
...
while running:
    ...
    if (pygame.Vector2(wolf1.position) - pygame.Vector2(destination)).magnitude_squared() == 0:
        ...
        for i in range(wolf1.tile_eval.size):
            wolf1.tile_eval.tiles[i] = (0,0)
    screen.fill(colour)
    for x in range(-8,8):
        for y in range(-8,8):
            tile = wolf1.position + pygame.Vector2(x,y)
            if keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT] or keys[pygame.K_TAB]:
                if (wolf1.tile_eval.value(tile)) != None:
                    rectangle = pygame.Rect(32*(x+8), 32*(y+8), 32, 32)
                    tile_colour = find_colour(wolf1.tile_eval.values, wolf1.tile_eval.value(tile), "traffic light")
