...
class animal(entity):
    def __init__(self, position, board):
        ...
        self.tile_eval = tile_eval(64, [self.position], [0], [0])
...
def normalise_list(num_list):
    ...
    m = min(num_list)
    try:
        scaling_factor = 1/(M-m)
    except ZeroDivisionError:
        return num_list
    return [(num_list[i]-m)*scaling_factor for i in range(len(num_list))]
    ...
    for x in range(-8,8):
        for y in range(-8,8):
            ...
            if keys[pygame.K_TAB]:
                if tile in wolf1.tile_eval.tiles:
...
while running:
    ...
    if wolf1.position == destination:
        ...
        destination = (wolf1.position[0]+random.randint(-100,100), wolf1.position[1]+random.randint(-100,100))
