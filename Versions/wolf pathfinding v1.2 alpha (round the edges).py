...
class animal(entity):
    ...
    def go_towards_destination(self, board, destination):
        ...
        self.previous_direction = best_directions[0] + 1
    ...
    def pathfind(self,board, destination):
        if 1/(1+distance_squared(self.position,destination)) <= self.best_value:
            ...
            if self.position == self.best_tile and self.previous_direction == self.travel_direction:
                self.give_up = True
        else:
            self.going_forward = True
            self.perimeter_length = 0
...
while running:
    ...
    if wolf1.position == destination or wolf1.give_up:
        wolf1.best_value = 0
        wolf1.give_up = False
        print(count)
        count = 0
