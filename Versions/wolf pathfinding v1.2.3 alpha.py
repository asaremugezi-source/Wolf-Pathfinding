...
class wolf(animal):
    def __init__(self, position, board):
        ...
        self.reversing = False
    def reset_bias(self):
        for index in range(1,len(self.pack)):
            if pack[index] == self and self.pack[index-1].best_tile == self.best_tile:
                self.bias = -1*self.pack[index-1].bias
            
                
    def find_sub_pack(self):
        self.sub_pack = []
        for wolf in self.pack:
            if wolf.best_tile == self.best_tile:
                self.sub_pack.append(wolf)
    
    def update(self, board):
        for wolf in self.pack:
            if wolf.give_up:
                self.give_up = True
        if not board.fetch_tile(find_tile(self.position, self.wall_direction)) and self.reverse and self.travel_direction not in [self.best_direction, self.best_direction-self.bias]:
            self.travel_direction = (self.travel_direction + 2)%4
            self.bias *= -1 
            self.reverse = False
            return
        for wolf in self.sub_pack:
            if wolf.best_value > self.best_value and wolf.bias == -1*self.bias:
                if self == self.pack[0]:
                    print(1)
                else:
                    print(2)
    ...
    for x in range(-8,9):
        for y in range(-8,9):
            ...
            if not game_board.fetch_tile(tile):
                if keys[pygame.K_TAB] and not animal.going_forward and ([x,y] == displacements[1] or [x,y] == displacements[0]):
            ...
            if tile == wolf3.position:
                pygame.draw.circle(screen, "grey", (32*(x+8.5), 32*(y+8.5)), 10)
    pygame.display.flip()
...
game_board = board(.3)        
wolf1 = wolf((1,7), game_board)
wolf2 = wolf((1,7), game_board)
wolf3 = wolf((1,7), game_board)
pack = [wolf1, wolf2, wolf3]
for wolf in pack:
    wolf.pack = pack
...
while running:
    ...
    if keys[pygame.K_PERIOD] and step == 0:
        step = True
        start = True
    ...
    if start:
        for wolf in pack:
            if wolf.best_value != 1:
                wolf.pathfind(game_board, destination)
            wolf.update(game_board)
            wolf.find_sub_pack()
            wolf.reset_bias()
    ...
    if dest_count > 10:
        break
    if wolf1.give_up or (wolf1.position == destination and wolf2.position == destination):
        wolf1.best_value = 0
        wolf2.best_value = 0
        wolf3.best_value = 0
    ...
    if keys[pygame.K_ESCAPE]:
        ...
        break
    #display_game()
    if (wolf1.best_tile == wolf2.best_tile and wolf1.best_tile != best_tile) or keys[pygame.K_z]:
        print(wolf1.best_tile, wolf2.best_tile, wolf3.best_tile, destination)
        best_tile = wolf1.best_tile
